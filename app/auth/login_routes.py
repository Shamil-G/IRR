...
# -----------------------------
@router.get("/logout")
async def logout(request: Request, user: SSO_User = Depends(login_required)):
    ip = ip_addr(request)
    username = request.session.get('username','')
    # return RedirectResponse("/login", status_code=302)
    session = request.session
    request.session.clear()
    log.info(f"LOGOUT. IP: {ip}, SESSION: {session}")
    ...
    if resp.status_code != 200:
        log.info(
            f"LOGOUT ERROR: {resp.status_code} при закрытии сессии {username} на SSO"
        )
